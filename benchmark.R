setwd("C:\\Users\\Charles\\Desktop\\Coursera\\Data Science JHU (10 parts)\\10. Capstone\\Coursera-Data-Science-Capstone")
...
predict.baseline <- function(input_text, complete_dt){
    ...
    output <- data.frame("Top_Choices" = c(matched_row$top1, matched_row$top2, matched_row$top3, matched_row$top4, matched_row$top5), 
                         "Scores" = c(matched_row$score1, matched_row$score2, matched_row$score3, matched_row$score4, matched_row$score5)) 

    return(c(as.character(output$Top_Choices[1]), as.character(output$Top_Choices[2]), as.character(output$Top_Choices[3])))
}
